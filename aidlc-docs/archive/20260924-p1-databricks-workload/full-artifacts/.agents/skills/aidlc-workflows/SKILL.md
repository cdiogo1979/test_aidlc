...
After all existing phases complete, follow `references/common/artifact-compaction.md` for Artifact Compaction / Intent Closure when verification is successful and approved, or when the user explicitly accepts a documented verification limitation under the waiver path. A waiver never represents successful verification. Do not compact while any stage approval gate remains pending. Existing phase behavior and Git branch/unit merge semantics remain unchanged.
