...
### Canonical, Current, and Historical Artifacts
- **Canonical project knowledge**: Current authoritative project facts under `aidlc-docs/knowledge/`.
- **Active workflow artifacts**: Intent-specific working documents under inception/construction while an intent is open.
- **Intent summary**: Concise account of a closed intent stored with its archive.
- **Intent archive**: Full traceability snapshot under `aidlc-docs/archive/<intent-id>/`; not loaded by default.
- **Artifact Compaction / Intent Closure**: Final step after passed verification or an explicit user-approved verification waiver that promotes curated durable knowledge, creates a summary, archives originals, clears the active workflow surface, and closes intent state. A waiver documents accepted unverified scope and is not a pass.
