...
2. If lifecycle metadata exists, load canonical documents in `aidlc-docs/knowledge/` and the active intent artifacts needed for the current stage. Do not enumerate or load `aidlc-docs/archive/` by default; retrieve a specific summary/artifact only when relevant. Legacy state without lifecycle metadata follows existing stage-based continuity.
